...
function HowItWorks({ image, steps }: HowItWorksProps) {
    ...
    const handleRightArrowClick = () => {
        setCurrentStep(prevStep => {
            ...
            showButton ? setShowButton(true) : setShowButton(nextStep === steps.length - 1); // Show button if it's the last step
            return nextStep;
        });
    };
    ...
    const renderLeftSideContent = () => {
        ...
        return (
            <img src={image.url} alt="icon" className="card-icon" />
        );
    };
        ...
        <div className="blurred">
            <div className="content">
                ...
                <div className="arrow-container">
                    <i className="fa-solid fa-arrow-left arrow left-arrow" onClick={handleLeftArrowClick}></i>
                    <div className="button-container">
                        {renderButton()}
                    </div>
                    <i className="fas fa-arrow-right arrow right-arrow" onClick={handleRightArrowClick}></i>
                </div>
            </div>
        </div>
    );
